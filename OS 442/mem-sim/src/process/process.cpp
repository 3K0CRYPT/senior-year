...
size_t Process::size() const { return num_bytes; }

bool Process::is_valid_page(size_t index) const {
  if(pages[index] != NULL) return true;
  else return false;
}
...
size_t Process::get_rss() const {
	...
	for(int i = 0; i < page_table.rows.size(); i++) if (page_table.rows[i].present) presentProcesses++;
...
double Process::get_fault_percent() const {
	...
	if(memory_accesses != 0) faultPercent= ((double)page_faults / (double)memory_accesses)*100;
  return faultPercent;
}
